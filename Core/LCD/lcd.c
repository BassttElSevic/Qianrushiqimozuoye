...
/* lcd_ex.c 相关的 LCD 扩展 IC 初始化函数等放在该文件中，由 lcd.c 等文件引用
 * 为了模块化管理，我们只在头文件中声明这些函数，然后在这里包含头文件。
 * 切记不要直接 #include "lcd_ex.c"，否则会导致同一函数在多个目标文件中被重复定义，
 * 链接时出现 multiple definition 错误。
 */
#include "lcd_ex.h"


SRAM_HandleTypeDef g_sram_handle;    /* SRAM 句柄 (用于驱动 LCD) */

/* LCD 前景色/背景色 */
uint32_t g_point_color = 0XF800;    /* 默认前景色 (红) */
uint32_t g_back_color  = 0XFFFF;    /* 默认背景色 (白) */
...
void Display_picture(uint16_t x, uint16_t y, uint16_t row, uint16_t column, unsigned short *pic)
{
    ...
    for (x1 = 0 + row;x < row + x1;x++) {
        for (y1 = 0 +column;y < column + y1;y++) {
            lcd_draw_point(x,y,*data++);
        }
    }
}

void show_picture(uint16_t x, uint16_t y, uint16_t width, uint16_t height, unsigned short *pic)
{
    uint16_t i, j;
    unsigned short *data = pic;

    for (j = y; j < height + y; j++) {
        for (i = x; i < width + x; i++) {
            lcd_draw_point(i, j, *data++);
        }
    }
}